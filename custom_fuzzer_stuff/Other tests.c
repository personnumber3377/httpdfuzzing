...
size_t bullshit(my_mutator_t *data, uint8_t *buf, size_t buf_size,
        u8 **out_buf, size_t max_size){
...
size_t incremental(my_mutator_t *data, uint8_t *buf, size_t buf_size,
	...
	//Incremental stage
		...
		printf("new_size, mutated_size: %d, %d\n", new_size, mutated_size);
		if(new_size > mutated_size){
			mutated_size = new_size;
		}
...
size_t afl_custom_fuzz(my_mutator_t *data, uint8_t *buf, size_t buf_size,
	...
	*/
	...
	//return bullshit(data, buf, buf_size, out_buf, max_size);
	//printf("buf: %s", *buf);
	if (*buf) {
		printf("buf %s\n",*buf);
	}
